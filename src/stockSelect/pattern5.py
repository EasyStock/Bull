...
class CStockPattern5(object):
    ...
    def GetStockData(self,startDay,threshold):
        sql1 = f'''
        SELECT B.`日期`,A.`股票代码`,A.`股票简称`,B.`收盘价` ,B.`最低价`,B.`成交量` FROM stock.stockbasicinfo AS A,(SELECT `日期`,`股票代码`,`收盘价`,`最低价`,`成交量` FROM stock.stockdailyinfo_2023 where `日期` >= "{startDay}" UNION SELECT `日期`,`股票代码`,`收盘价` ,`最低价`,`成交量` FROM stock.stockdailyinfo where `日期` >= "{startDay}") AS B where A.`股票代码` = B.`股票代码`
        '''
        ...
        for stockID, group in groups:
            ...
            #df["昨日收盘价"] = df["MA20"].astype("float")
            #df["涨跌幅"] = df["MA20"].astype("float")
