...
class CFetchBanKuaiData(object):
    def __init__(self,dbConnection,today):
        ...
        self.payload = {
                ...
                "question": f'''{self.today.replace("-",".")} 板块指数代码 指数简称 开盘价  收盘价 最高价 最低价 成交量 成交额 涨跌幅 量比 换手率 上涨家数 下跌家数 流通市值 总市值''',
                "perpage": 100,
                "page": 1,
    ...
    def RequestAllPagesDataAndWriteToDB(self,perPage=100):
        ...
        sqls = self._DataFrameToSqls_INSERT_OR_REPLACE(self.dataFrame,"bankuai_index_dailyinfo")
        for sql in sqls:
