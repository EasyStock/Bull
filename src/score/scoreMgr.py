...
class CScoreMgr(object):
    ...
    def _Quantile(self):
        if self.ZhuanZaiInfo is None:
            return 
        newDf = pd.DataFrame(self.ZhuanZaiInfo,columns=["成交额(万元)","总市值（亿元)","剩余规模"])
    ...
    def CalcMakingValueScore(self):
        #计算市值分数值
        if self.ZhuanZaiInfo is None:
            return 
        
        self.ZhuanZaiInfo["市值分数"] = self.ZhuanZaiInfo.apply(lambda row: self._score(row["总市值（亿元)"],self.QuantileDf["总市值（亿元)"],True), axis=1)
    ...
    def WriteToDB(self,date):
        ...
        for index, row in self.ZhuanZaiInfo.iterrows():
            ...
            sql = f'''REPLACE INTO `stock`.`kezhuanzai_score` (`日期`, `转债代码`, `成交量分数`, `抗跌分数`, `活跃度分数`,`波动率分数`,`总分`) VALUES ('{date}', '{stockID}', '{v}', '{d}', '{z}',{s},{total});'''
            #print(sql)
    ...
    def Select(self,date):
        sql = f'''select A.`转债代码`,B.`转债名称`,A.`总分`from kezhuanzai_score As A,kezhuanzhai AS B where A.`日期` = "{date}" and B.`日期` = "{date}"  and A.`转债代码` = B.`转债代码` and A.`总分` >5 order by A.`总分` DESC;'''
        results, columns = self.dbConnection.Query(sql)
        df = pd.DataFrame(results,columns=columns)
    
        fodler = f'{workSpaceRoot}/复盘/可转债/{date}/'
